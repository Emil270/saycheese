<?php
	...
	*/
	...
	function Select($table, $params = [],  $limit = 0, $offset = 0, $desc = 0)
	{
		global $connection;
	...
	*/
	...
	function SelectClientOfOrder($table1, $table2, $id){
		global $connection;
		$sql = "SELECT t2.name, t2.surname, t1.email FROM $table1 AS t1 JOIN $table2 AS t2 ON t1.id = t2.id_user WHERE t1.id = $id";
		$query = $connection->prepare($sql);
		$query->execute();
		$result = $query->fetch();
		return $result;
	...
	function SelectAllModerators($table1, $table2, $params = []){
		global $connection;
		$sql = "SELECT t1.id, t1.email, t2.name, t2.surname, t2.avatar FROM $table1 AS t1 JOIN $table2 AS t2 ON t1.id = t2.id_user";
		if(!empty($params)){
			$sql = $sql . GetSqlWhere($params);
		}
